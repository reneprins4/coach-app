...
import type { ForecastSession, RegressionResult, ForecastChartPoint, ForecastResult } from '../types'
 ...
 */
export function weightedLinearRegression(points: RegressionPoint[], smoothingFactor: number = SMOOTHING_FACTOR): RegressionResult {
 ...
 */
export function calculateTrainingFrequency(sessions: ForecastSession[]): number {
 ...
 */
export function calculateForecast(sessions: ForecastSession[]): ForecastResult {
  ...
  const points: RegressionPoint[] = sessions.map((s, i) => ({
    x: i,
