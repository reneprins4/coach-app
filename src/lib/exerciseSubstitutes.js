...
// Direct substitution map: exercise name → { reason → substitute name }
// Falls back to muscle group lookup when not listed
const DIRECT_SUBSTITUTES = {
  // ---- CHEST ----
...
// All exercises as a flat reference list (for fallback lookup)
const ALL_EXERCISES = [
  // arms
  ...
  // chest
  { name: 'Cable Fly',                 muscle_group: 'chest',     equipment: 'bodyweight' },
  ...
  { name: 'Incline Bench Press',       muscle_group: 'chest',     equipment: 'bodyweight' },
  ...
  { name: 'Leg Curl',                  muscle_group: 'legs',      equipment: 'bodyweight' },
 ...
 * Picks the best available exercise based on equipment priority.
 */
function findFallbackSubstitute(originalName, muscleGroup, reason, equipment) {
  ...
  // Filter candidates: same muscle group, available equipment, not the original
  ...
  candidates.sort((a, b) => {
    const ai = priority.indexOf(a.equipment)
    const bi = priority.indexOf(b.equipment)
...
/**
 * Get exercise info from our database by name.
 */
...
/**
 * Estimate weight for a substitute based on the original weight.
 * Different equipment types have different weight equivalencies.
 */
function estimateSubstituteWeight(originalWeight, originalEquipment, substituteEquipment) {
  if (!originalWeight || originalWeight === 0) return 0

  // Dumbbell weights are roughly 60-70% of barbell (per hand, so total similar)
  // Cable weights can be similar to dumbbell
  const conversions = {
    'barbell->dumbbell': 0.65,
    'dumbbell->barbell': 1.4,
    'barbell->cable':    0.7,
    'cable->barbell':    1.3,
    'barbell->bodyweight': null, // bodyweight = 0
  ...
  if (factor !== undefined) return Math.round((originalWeight * factor) / 2.5) * 2.5

  return originalWeight // same equipment type → same weight
}
 ...
 */
export function getExerciseSubstituteLocal({ exercise, reason, equipment, experienceLevel, bodyweight }) {
  const normalizedReason = reason?.toLowerCase().replace(/\s+/g, '_') || 'default'
  const reasonKey = ['machine_busy', 'no_equipment', 'injury'].includes(normalizedReason)
    ? normalizedReason
    : 'machine_busy'
  ...
  // 3. Last resort: same exercise (shouldn't happen)
