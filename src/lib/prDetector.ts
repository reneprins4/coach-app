...
export function calculateE1RM(weight: number, reps: number): number {
  // Robust guard against NaN, Infinity, null, undefined, negative, and zero values
...
export function detectPR(
  ...
  weight: number,
  reps: number,
  historicalSets: Pick<WorkoutSet, 'exercise' | 'weight_kg' | 'reps'>[]
 ...
 */
export function computeAllPRs(workouts: Workout[]): Map<string, PRRecord> {
 ...
 */
export function sortPRsForDisplay(prsMap: Map<string, PRRecord>): PRDisplayRecord[] {
  const records: PRDisplayRecord[] = []
