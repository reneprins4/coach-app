...
import type { Workout, InjuryRisk, InjuryRiskLevel } from '../types'
 ...
 */
export function calculateWeeklyVolume(workouts: Workout[], weeksBack: number = 1): Record<string, number> {
  const now = new Date()
 ...
 */
export function detectVolumeSpikes(workouts: Workout[]): InjuryRisk[] {
 ...
 */
export function detectMuscleImbalances(workouts: Workout[]): InjuryRisk[] {
  const risks: InjuryRisk[] = []
 ...
 */
export function detectConsecutiveHighRPE(workouts: Workout[]): InjuryRisk[] {
  const risks: InjuryRisk[] = []
  const muscleRPESessions: Record<string, { consecutive: number; hasDeload: boolean }> = {}
  
  // Sort workouts by date (newest first is already the case)
  const sortedWorkouts = [...workouts].sort(
    (a: Workout, b: Workout) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
  )
 ...
 */
export function detectInsufficientRecovery(workouts: Workout[]): InjuryRisk[] {
  const risks: InjuryRisk[] = []
  const muscleWarnings: Record<string, number> = {}
  
  // Sort workouts chronologically
  const sorted = [...workouts].sort(
    (a: Workout, b: Workout) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
  )
  ...
  for (const w of sorted) {
    ...
    for (const s of (w.workout_sets || [])) {
      ...
      if (lastTrained[muscle]) {
        const hoursSince = (workoutDate.getTime() - lastTrained[muscle]!.getTime()) / 3600000
 ...
 */
export function analyzeInjuryRisks(workouts: Workout[]): InjuryRisk[] {
  ...
  const levelOrder: Record<InjuryRiskLevel, number> = { hoog: 0, matig: 1 }
...
export default function InjuryRadar({ workouts }: { workouts: Workout[] }) {
  ...
  const getMuscleLabel = (muscle: string): string => {
    ...
    return (MUSCLE_NL as Record<string, string>)[muscle] || muscle
  }
