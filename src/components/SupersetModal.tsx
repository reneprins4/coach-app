...
import { calculateTimeSavings } from '../lib/supersetArchitect'
import { useModalA11y } from '../hooks/useModalA11y'
import type { SupersetModalProps, SupersetGroupResult } from '../types'
import type { TFunction } from 'i18next'

export default function SupersetModal({ exercises, onApply, onClose }: SupersetModalProps) {
  const { t } = useTranslation()
  const [confirmed, setConfirmed] = useState(false)

  const savings = useMemo(() => calculateTimeSavings(exercises), [exercises])
  const { supersets, savedMinutes, savedPercent, hasSupersets } = savings
  ...
  if (!hasSupersets) {
    return (
      <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/70 p-4">
        <div role="dialog" aria-modal="true" aria-labelledby="superset-modal-title-empty" className="w-full max-w-sm rounded-2xl bg-gray-900 p-6">
          <div className="mb-4 flex items-start justify-between">
            <div>
              <h3 id="superset-modal-title-empty" className="text-xl font-black tracking-tight text-white">{t('superset_modal.title')}</h3>
              <p className="mt-0.5 label-caps">{t('superset_modal.no_pairs')}</p>
            </div>
            <button onClick={onClose} aria-label={t('common.close') || 'Close'} className="text-gray-600 active:text-gray-400 min-h-[44px] min-w-[44px] flex items-center justify-center">
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/70 p-4">
      <div role="dialog" aria-modal="true" aria-labelledby="superset-modal-title" className="max-h-[88vh] w-full max-w-sm flex flex-col rounded-2xl bg-gray-900 overflow-hidden">
        {/* Header */}
        <div className="flex items-start justify-between px-5 pt-5 pb-4 border-b border-gray-800 shrink-0">
          <div>
            <h3 id="superset-modal-title" className="text-xl font-black tracking-tight text-white">{t('superset_modal.title')}</h3>
            <p className="mt-1 label-caps text-cyan-500">
              -{savedMinutes} min · {savedPercent}% {t('superset_modal.faster')}
            </p>
          </div>
          <button onClick={onClose} aria-label={t('common.close') || 'Close'} className="text-gray-600 active:text-gray-400 mt-0.5 min-h-[44px] min-w-[44px] flex items-center justify-center">
            <X size={20} aria-hidden="true" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-2">
          {supersets.map((group, idx) => (
            <GroupCard key={idx} group={group} t={t} />
          ))}
        </div>
...
function GroupCard({ group, t }: { group: SupersetGroupResult; t: TFunction }) {
