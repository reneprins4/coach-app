...
import { getExerciseGuide } from '../lib/ai'
import { useModalA11y } from '../hooks/useModalA11y'
import type { ExerciseGuideProps, ExerciseGuideResponse } from '../types'

export default function ExerciseGuide({ exercise, onClose }: ExerciseGuideProps) {
  const { t, i18n } = useTranslation()
  const [guide, setGuide] = useState<ExerciseGuideResponse | null>(null)
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    getExerciseGuide(exercise.name, i18n.language)
      .then(data => {
        if (!cancelled) {
          setGuide(data)
          setLoading(false)
    ...
    <div className="fixed inset-0 z-[60] flex items-end" onClick={onClose}>
      ...
      >
        ...
        <div className="mb-5 flex items-start justify-between gap-3">
          <div>
            <h2 id="exercise-guide-title" className="text-lg font-bold text-white">{exercise.name}</h2>
        ...
        {loading && (
          <div className="flex flex-col items-center gap-3 py-10" role="status" aria-live="polite">
            <div className="h-8 w-8 animate-spin rounded-full border-2 border-gray-700 border-t-cyan-500" aria-hidden="true" />
