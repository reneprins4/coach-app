      ...
      <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/70 p-4">
        <div className="w-full max-w-sm rounded-2xl bg-gray-900 p-6">
          <div className="mb-4 flex items-start justify-between">
            <div>
              <h3 className="text-xl font-black tracking-tight text-white">Superset modus</h3>
              <p className="mt-0.5 label-caps">Geen koppelbaar paar gevonden</p>
            </div>
          ...
          <p className="mb-6 text-sm text-gray-400 leading-relaxed">
            Supersets werken het best met antagonist spiergroepen — borst + rug, biceps + triceps. 
            Voeg meer oefeningen toe om ze te combineren.
          </p>
        ...
        <div className="flex items-start justify-between px-5 pt-5 pb-4 border-b border-gray-800 shrink-0">
          <div>
            ...
            <p className="mt-1 label-caps text-cyan-500">
              -{savedMinutes} min · {savedPercent}% sneller
            </p>
          </div>
          ...
          {!confirmed ? (
            ...
            <div className="space-y-3">
              <p className="text-center text-sm text-gray-500 leading-relaxed">
                Wissel A en B zonder rust. Rust na elke volledige ronde.
              </p>
  ...
  if (group.type === 'superset') {
    ...
    return (
      <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-4">
        <p className="mb-3 label-caps text-cyan-500">Superset</p>
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-bold tracking-tight text-white">{a.name}</p>
              <p className="label-caps">{setsA} sets</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="label-caps">{setsB} sets</p>
            </div>
            <span className="label-caps text-cyan-500/60">B</span>
          </div>
        </div>
        {group.restAfter && (
          <p className="mt-3 label-caps">{group.restAfter}s rust na elke ronde</p>
        )}
      </div>
    )
  }
  ...
  return (
    <div className="rounded-2xl border border-gray-800 bg-gray-800/40 px-4 py-3 flex items-center justify-between">
      <div>
        ...
        <p className="label-caps">{sets} sets · {group.restAfter}s rust</p>
      </div>
      <span className="label-caps text-gray-600">Solo</span>
    </div>
  )
}
