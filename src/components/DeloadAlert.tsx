...
export default function DeloadAlert({ workouts, settings, updateSettings }: DeloadAlertProps) {
  ...
  const getSignalLabel = (signal: FatigueSignal): string | null => {
    switch (signal.type) {
      case 'rpe_drift':
      ...
      case 'frequency_drop':
        return t('deload.signal_frequency', { count: Number(signal.perWeek) })
      default:
        return null
    }
  }

  const getSignalIcon = (type: FatigueSignal['type']) => {
    switch (type) {
